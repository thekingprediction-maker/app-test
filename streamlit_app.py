...
<head>
    ...
    <style>
        ...
        .debug-box { background: #1e293b; border: 1px solid #475569; border-radius: 8px; padding: 10px; margin-top: 10px; font-size: 11px; color: #94a3b8; font-family: monospace; }
    </style>
</head>
...
let SEASON = 2024; // API-Sports usa 2024 per la stagione 2024/2025 in corso
...
function setError(msg, debugInfo = "") {
    ...
    if (debugInfo) {
        html += `<div class="debug-box">Debug: ${debugInfo}</div>`;
    }
...
// ===================== ESTRAZIONE STATISTICHE DA FIXTURES =====================
// API-Sports team statistics ha una struttura diversa: le stats sono per fixture, non aggregate
// Dobbiamo usare /fixtures?team=ID&season=SEASON&league=LEAGUE per ottenere le partite
// e poi /fixtures/statistics?fixture=ID per ogni partita
async function getTeamStatsFromFixtures(teamId) {
    try {
        // 1. Ottieni tutte le partite della squadra nella stagione
        const fixturesUrl = `https://v3.football.api-sports.io/fixtures?team=${teamId}&season=${SEASON}&league=${currentLeague}`;
        ...
        const fixtures = fixturesData.response.filter(f => f.fixture.status.short === 'FT').slice(0, 10);
        
        // 2. Per ogni partita, ottieni le statistiche
...
async function runDeepAnalysis() {
    ...
    try {
        ...
        // Fetch parallelo: statistiche da fixtures + cartellini
        ...
        if (!statsH || !statsA) {
            setError("Statistiche non disponibili per una o entrambe le squadre", 
                     `StatsH: ${JSON.stringify(statsH)}, StatsA: ${JSON.stringify(statsA)}`);
            return;
        }
        ...
        // Per i corner subiti, usiamo una stima: corner fatti avversario ~ corner subiti
            ...
            <div class="res-box border-l-red-500">
                ...
                <div class="grid grid-cols-2 mt-2 pt-2 border-t border-slate-800">
                    <div>
                        ...
                        <p class="stat-detail">Media: ${foulsForH.toFixed(1)} | Arbitro: ${refVal}</p>
                    </div>
                    <div class="text-right">
                        <p class="label-spread">Ospite</p>
                        <p class="text-xl teko text-red-400">${fA.toFixed(2)} ${getAdvice(fA, 'sprFoulsA')}</p>
                        <p class="stat-detail">Media: ${foulsForA.toFixed(1)} | Arbitro: ${refVal}</p>
                    </div>
                </div>
            </div>`;
        }
        ...
        <div class="res-box" style="border-left-color: #3b82f6;">
            ...
            <p class="text-xs text-slate-500 mt-1">Dati da API-Sports.io (ultime ${statsH.matches} partite)</p>
        </div>`;
