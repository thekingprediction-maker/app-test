...
<body class="p-4 md:p-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="card-premium mb-8">
            ...
            <button onclick="runDeepAnalysis()" class="btn-analizza shadow-xl">Genera Consigli di Scommessa</button>
        </div>
...
async function runDeepAnalysis() {
    ...
    resDiv.innerHTML = "<div class='text-center py-20 animate-pulse text-blue-500 font-black teko text-4xl'>CALCULATING ODDS...</div>";
    ...
    try {
        ...
        const tT = (rH.response.shots.total.average * (xGH/0.11) * 1.05) + (rA.response.shots.total.average * (xGA/0.11));
        const tP = (rH.response.shots.on_goal.average * (xGH/0.11) * 1.05) + (rA.response.shots.on_goal.average * (xGA/0.11));
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="res-box">
                    <div class="flex justify-between items-start mb-4">
                        <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Tiri Totali Match</p>
                        <span class="advice-pill ${advT.class}">${advT.type} 23.5</span>
                    </div>
                    <h2 class="text-6xl font-black teko text-white mb-2">${tT.toFixed(2)}</h2>
                    <p class="text-emerald-400 font-black text-2xl teko">PROBABILITÀ: ${advT.p.toFixed(1)}%</p>
                </div>

                <div class="res-box border-l-purple-500">
                    <div class="flex justify-between items-start mb-4">
                        <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest">In Porta Match</p>
                        <span class="advice-pill ${advP.class}">${advP.type} 8.5</span>
                    </div>
                    <h2 class="text-6xl font-black teko text-white mb-2">${tP.toFixed(2)}</h2>
                    <p class="text-purple-400 font-black text-2xl teko">PROBABILITÀ: ${advP.p.toFixed(1)}%</p>
                </div>
            </div>

            <div class="p-6 bg-blue-600/10 border border-blue-500/20 rounded-2xl text-center">
                <p class="text-xs font-bold text-blue-400 uppercase tracking-[0.2em]">Consiglio AI: ${(advT.p > 65 || advP.p > 65) ? 'ALTA CONFIDENZA - OTTIMO VALORE' : 'CAUTELA - DATI ENTRO IL RANGE MEDIO'}</p>
            </div>
        `;
    } catch(e) { resDiv.innerHTML = "Errore API 2025: Dati non sufficienti."; }
}
</script>
</body>
</html>
